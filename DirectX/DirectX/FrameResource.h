...
struct FrameResourceMaterialWaves
{
public:
	FrameResourceMaterialWaves(
		ID3D12Device* device, UINT passCount, UINT objectCount, UINT materialCount, UINT waveVertCount);
	FrameResourceMaterialWaves(const FrameResourceMaterialWaves& rhs) = delete;
	FrameResourceMaterialWaves& operator=(const FrameResourceMaterialWaves& rhs) = delete;
	~FrameResourceMaterialWaves();
	...
	// 파도 정점 버퍼
	std::unique_ptr<UploadBuffer<VertexLight>> WavesVB = nullptr;
