...
Chap7_2App::Chap7_2App(HINSTANCE hInstance) : D3DApp(hInstance)
{
}

Chap7_2App::~Chap7_2App()
{
	if (md3dDevice != nullptr)
		FlushCommandQueue();
}

bool Chap7_2App::Initialize()
{
...
void Chap7_2App::Update(const GameTimer& gt)
{
	OnKeyboardInput(gt);
	UpdateCamera(gt);
...
void Chap7_2App::Draw(const GameTimer& gt)
{
	// 커맨드 리스트 할당자 리셋
...
void Chap7_2App::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void Chap7_2App::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void Chap7_2App::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void Chap7_2App::OnKeyboardInput(const GameTimer& gt)
{
...
void Chap7_2App::UpdateCamera(const GameTimer& gt)
{
...
void Chap7_2App::UpdateObjectCBs(const GameTimer& gt)
{
...
void Chap7_2App::UpdateMainPassCB(const GameTimer& gt)
{
...
void Chap7_2App::UpdateWaves(const GameTimer& gt)
{
...
void Chap7_2App::BuildShadersAndInputLayout()
{
...
void Chap7_2App::BuildLandGeometry()
{
	// 기본 메시 생성
	...
	for (size_t i = 0; i < grid.Vertices.size(); ++i)
	{
		...
		{
			vertices[i].Color = XMFLOAT4(1.0f, 0.95f, 0.62f, 1.0f);		// 모래색
		}
		else if (vertices[i].Pos.y < 5.0f)
		{
			vertices[i].Color = XMFLOAT4(0.48f, 0.77f, 0.46f, 1.0f);	// 풀색
		}
		else if (vertices[i].Pos.y < 12.0f)
		{
			vertices[i].Color = XMFLOAT4(0.1f, 0.48f, 0.19f, 1.0f);		// 나뭇잎색
		}
		else if (vertices[i].Pos.y < 20.0f)
		{
			vertices[i].Color = XMFLOAT4(0.45f, 0.39f, 0.34f, 1.0f);	// 짙은 갈색
		}
		else
		{
			vertices[i].Color = XMFLOAT4(DirectX::Colors::White);		// 눈색
		}
	}
...
void Chap7_2App::BuildWavesGeometryBuffers()
{
...
void Chap7_2App::BuildFrameResources()
{
	for (int i = 0; i < gNumFrameResources; ++i)
	{
		mFrameResources.push_back(
			std::make_unique<FrameResourceWaves>(
				md3dDevice.Get(), 1, (UINT)mAllRitems.size(), mWaves->VertexCount())
		);
	}
}

void Chap7_2App::BuildRenderItems()
{
	auto wavesRitem = std::make_unique<RenderItemApp7>();
...
void Chap7_2App::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItemApp7*>& ritems)
{
...
float Chap7_2App::GetHillsHeight(float x, float z) const
{
	return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

XMFLOAT3 Chap7_2App::GetHillsNormal(float x, float z) const
{
	XMFLOAT3 n(
